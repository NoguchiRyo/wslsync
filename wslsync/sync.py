...
class WSLSyncEngine:
        ...
        for file_rel_path in self.config.files:
            ...
            if source_path.is_file():
                ...
                try:
                    ...
                    raise PermissionError(f"Permission denied copying {source_path} to {dest_path}") from e
                except OSError as e:
                ...
                try:
                    ...
                    raise PermissionError(f"Permission denied copying directory {source_path} to {dest_path}") from e
                except OSError as e:
                    raise OSError(f"Failed to copy directory {source_path} to {dest_path}") from e
    ...
    def validate_paths(self) -> bool:
        ...
        if not self.config.windows_base.exists():
            raise FileNotFoundError(f"Source directory not found: {self.config.windows_base}")
        
        if not self.config.wsl2_base.exists():
            raise FileNotFoundError(f"Destination directory not found: {self.config.wsl2_base}")
        
        # Check if paths are accessible
        try:
            list(self.config.windows_base.iterdir())
        except PermissionError as e:
            raise PermissionError(f"Cannot access source directory: {self.config.windows_base}") from e
        
        try:
            list(self.config.wsl2_base.iterdir())
        except PermissionError as e:
            raise PermissionError(f"Cannot access destination directory: {self.config.wsl2_base}") from e
    ...
    def create_directory_structure(self, file_path: Path) -> None:
        """
        Create necessary directory structure for a file path.
        ...
        try:
            ...
            raise PermissionError(f"Permission denied creating directory: {file_path.parent}") from e
        except OSError as e:
