...
def safe_copy_file(source: Path, destination: Path, preserve_timestamps: bool = True) -> bool:
    ...
    try:
        ...
        raise PermissionError(f"Permission denied copying {source} to {destination}") from e
    except OSError as e:
...
def get_sync_summary(copied_files: List[Path], deleted_files: List[Path]) -> str:
    ...
    summary = f"Sync completed: {copied_count} files copied, {deleted_count} files deleted"
