...
class TestWSLSync(unittest.TestCase):
    ...
    def test_source_files_exist(self):
        ...
        for file_path in expected_files:
            ...
            self.assertTrue(
                source_file.exists(), f"Source file {file_path} should exist"
            )
    ...
    def test_file_copying(self):
        ...
        for file_path in files_to_copy:
            ...
            self.assertEqual(
                source_content, dest_content, f"Content should match for {file_path}"
            )
    ...
    def test_cleanup_functionality(self):
        ...
        for file_path in cleanup_files:
            ...
            self.assertTrue(
                file_obj.exists(), f"Cleanup target {file_path} should exist initially"
            )
        ...
        for file_path in cleanup_files:
            file_obj = self.wsl2_dest / file_path
            self.assertFalse(
                file_obj.exists(), f"File {file_path} should be cleaned up"
            )
    ...
    def test_directory_structure_creation(self):
        ...
        test_files = ["documents/config.txt", "projects/app/main.py"]
        ...
        for file_path in test_files:
            ...
            self.assertTrue(
                dest.parent.exists(), f"Directory for {file_path} should exist"
            )
            self.assertTrue(
                dest.parent.is_dir(), f"Parent should be a directory for {file_path}"
            )
    ...
    def test_empty_destination_directory(self):
        ...
        # Copy files
        files_to_copy = ["documents/config.txt", "documents/notes.md"]
